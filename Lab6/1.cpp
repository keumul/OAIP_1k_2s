...
void add(list*&, float); //функция добавления элемента, передается адрес списка и символ, который добавляется 
float del(list*&, float); //функция удаления, передается адрес списка и символ, который удаляется 
int empty(list*); //функция, которая проверяет, пуст ли список
...
int main()
{
    ...
    while (choice != 6)
    {
        ...
        case 2:   if (!empty(first)) // удалить число из списка
        {
            ...
            break;
        case 3:   arifm(first);	// вычисление среднего арифмитического положительных элементов
            break;
        case 4:  toFile(first);
            break;
...
void menu(void)  //Вывод меню 
{
    ...
    cout << " 3 - Вычисление среднего арифмитического из положительных значений" << endl;
...
void arifm(list* p)   
{
    ...
    else
    {
        ...
        cout << "Среднее арифмитическое = " << (howmuch/sm)<< endl;
    }
}
